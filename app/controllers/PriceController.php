...
class PriceController {
    ...
    public function index(): void {
        ...
        $sql = "
          SELECT s.id, p.name AS product, p.unit, m.name AS market, m.region, s.price_rs, s.price_date,
    ...
    public function submit(): void {
        ...
        if ($_SERVER['REQUEST_METHOD']==='POST') {
            ...
            if ($errors) { view('prices/submit', compact('markets','products','errors')); return; }

            $u = Auth::user();
            $trusted = (int)$pdo->query("SELECT is_trusted FROM users WHERE id=".(int)$u['id'])->fetch()['is_trusted'];
            $status = $trusted ? 'approved' : 'pending';

            $stmt=$pdo->prepare("INSERT INTO price_submissions (user_id,market_id,product_id,price_rs,price_date,source,note,status,submitted_at) VALUES (?,?,?,?,?,?,?,?,NOW())");
            $stmt->execute([$u['id'],$marketId,$productId,$priceRs,$priceDate,$source,$note,$status]);
    ...
    public function predict(): void {
        ...
        if ($marketId>0 && $productId>0) {
            $stmt=$pdo->prepare("SELECT price_date, price_rs FROM price_submissions WHERE status='approved' AND market_id=? AND product_id=? ORDER BY price_date ASC");
            $stmt->execute([$marketId,$productId]);
            $rows=$stmt->fetchAll();

            foreach ($rows as $r) $history[]=['date'=>$r['price_date'],'price'=>(float)$r['price_rs']];
            ...
            $prediction = $this->callMl($app['ml_service_url'], $payload);
        }
    ...
    private function callMl(string $url, array $payload): ?array {
        ...
        if ($resp===false || $code!==200) return ['ok'=>false,'message'=>"ML indisponible: {$err} (HTTP {$code})"];
        $data=json_decode($resp,true);
        return $data ?: ['ok'=>false,'message'=>'Réponse ML invalide'];
    }
}
