<div class="page-header">
  <span class="section-num"><?php echo I18n::t('predict.section'); ?></span>
  <h1 class="page-title"><?php echo I18n::t('predict.title'); ?></h1>
  <p class="page-subtitle"><?php echo I18n::t('predict.subtitle'); ?></p>
</div>
  ...
  <form class="d-flex gap-2 align-items-end flex-wrap" method="get">
    <div style="min-width:240px; flex:1;">
      ...
      <select name="market_id" class="form-select" required>
        <option value=""><?php echo I18n::t('predict.select_market'); ?></option>
    ...
    <div style="min-width:240px; flex:1;">
      ...
      <select name="product_id" class="form-select" required>
        <option value=""><?php echo I18n::t('predict.select_product'); ?></option>
    ...
    <div>
      ...
      <button class="btn btn-primary" id="submitBtn">
        <i class="bi bi-graph-up-arrow me-1"></i><?php echo I18n::t('predict.analyze'); ?>
      </button>
    </div>
  </form>
</div>
...
<div class="row g-3">
  <div class="col-lg-7">
    <div class="card h-100" style="border-left:none;">
      <div class="card-header"><?php echo I18n::t('predict.history_title'); ?></div>
      <div class="card-body">
  ...
  <div class="col-lg-5">
    <div class="card-dark h-100" style="border-top:3px solid var(--teal); border-radius:14px;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem;">
        <span style="font-family:var(--f-sans);font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:var(--text-muted-light);"><?php echo I18n::t('predict.day7'); ?></span>
        <span class="chip chip-teal"
              data-bs-toggle="tooltip" data-bs-placement="top"
              title="<?php echo htmlspecialchars(I18n::t('predict.tooltip')); ?>">
          <i class="bi bi-cpu-fill"></i> Ridge ML
        </span>
      </div>

      <?php if (!$prediction): ?>
        <p class="text-muted mb-0"><?php echo I18n::t('predict.no_result'); ?></p>
      <?php elseif (!($prediction['ok'] ?? false)): ?>
        <div class="alert alert-warning mb-0"><?php echo htmlspecialchars($prediction['message'] ?? I18n::t('predict.error')); ?></div>
      <?php else: ?>
        <div style="font-family:var(--f-handwritten);font-size:1rem;color:var(--text-muted-light);margin-bottom:0.3rem;"><?php echo I18n::t('predict.estimated_price'); ?></div>
        ...
        <div style="display:flex;align-items:center;gap:1.5rem;margin-bottom:1.5rem;">
          ...
          <div>
            <div style="font-family:var(--f-sans);font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:0.10em;color:var(--text-muted-light);margin-bottom:0.3rem;"><?php echo I18n::t('predict.confidence'); ?></div>
        ...
        <div style="font-size:0.75rem;color:var(--text-muted-light);">
          <?php echo (int)$prediction['points']; ?> <?php echo I18n::t('predict.points_used'); ?>
        </div>

        <p class="predict-disclaimer">
          <?php echo I18n::t('predict.disclaimer'); ?>
        </p>
      <?php endif; ?>
    </div>
  </div>
